...
fn test_scale_validation_comprehensive() {
    ...
    for (scale, result) in &results {
        ...
        println!("Scale {}: Rust {:?}, Python {:?} (ratio: {:.2}x)", 
    ...
    if total_python_time > total_rust_time {
        ...
        if overall_ratio >= 3.0 {
            println!("🚀 Performance target ACHIEVED: {:.2}x >= 3.0x", overall_ratio);
    ...
    assert!(pass_count > 0, "At least some scales should pass validation");
}
...
fn test_performance_targets_migration() {
    ...
    assert!(result.meets_performance_target(1.0), 
            "Rust should be at least as fast as Python: ratio = {:.3}", result.performance_ratio);
...
fn test_memory_efficiency_validation() {
    ...
    for config in test_configs {
        ...
        println!("💾 Memory Test ({}): {}", result.config.count, result.summary());
        
        // Basic validation that the test completes successfully
        assert!(result.is_valid() || result.rust_duration < result.python_duration,
                "Memory test should pass or show performance improvement: {}", result.summary());
    }
}
...
fn test_error_handling_parity() {
    ...
    let result = validator.run_validation(config).expect("Basic validation should work");
    
    println!("🛡️  Error Handling Test: {}", result.summary());
    
    // Validate that error handling doesn't break the basic functionality
    assert!(result.is_valid() || result.rust_output.len() > 0,
...
fn test_deterministic_behavior() {
    ...
    let result1 = validator.run_validation(config1).expect("First run should succeed");
    let result2 = validator.run_validation(config2).expect("Second run should succeed");
    ...
    assert!(perf_diff < 10.0, 
            "Performance should be reasonably consistent between runs: diff = {:.3}", perf_diff);
}