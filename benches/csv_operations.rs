...
use opnsense_config_faker::io::csv::{write_csv, read_csv, write_csv_streaming, read_csv_streaming};
...
fn bench_csv_serialization(c: &mut Criterion) {
    ...
    for count in [100, 500, 1000, 2000].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("write_csv", count),
            count,
            |b, _| {
                b.iter(|| {
        ...
        group.bench_with_input(
            ...
            |b, _| {
                b.iter(|| {
                    let temp_file = NamedTempFile::new().unwrap();
                    write_csv_streaming(black_box(configs.clone().into_iter()), temp_file.path()).unwrap();
                })
            },
        );
    }
...
fn bench_csv_deserialization(c: &mut Criterion) {
    ...
    for count in [100, 500, 1000, 2000].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("read_csv", count),
            count,
            |b, _| {
                b.iter(|| {
...
fn bench_csv_round_trip(c: &mut Criterion) {
    ...
    for count in [100, 500, 1000].iter() {
        ...
        group.bench_with_input(
            BenchmarkId::new("round_trip", count),
            count,
            |b, _| {
                b.iter(|| {
                    let temp_file = NamedTempFile::new().unwrap();
...
criterion_group!(benches, bench_csv_serialization, bench_csv_deserialization, bench_csv_round_trip);
