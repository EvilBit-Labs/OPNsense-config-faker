...
├── legacy/               # Original Python implementation (preserved for reference)
└── opnsense-config.xsd   # OPNsense XML schema definition
```

### ⚠️ Generated Code Warning

**CRITICAL**: The `opnsense/models/` directory contains Python code generated from the OPNsense XSD schema using xsdata-pydantic. This is part of the legacy Python implementation and should not be modified for the Rust version.

- **Generated by**: `xsdata` command from `opnsense-config.xsd`
- **Legacy Code**: Part of the Python implementation, preserved for reference
- **Rust Models**: Use hand-written Rust models in `src/model/` for the new implementation
...
- **Preservation**: Original Python functionality preserved in `legacy/` directory for reference
- **Migration**: Core functionality being migrated to Rust with improved performance and safety
- **Modernization**: New implementation uses modern Rust practices, strong typing, and memory safety
...
## 6. AI Assistant Behavior and Rules of Engagement
...
## 7. Rust Implementation Features
